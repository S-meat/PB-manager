...
pb-manager/
  ...
  data/                 DB初期データ（ヨコレイ在庫_スプレッドシート投入用.xlsx）。再変換・再生成しないこと
  docs/                 仕様書・手順書一式
...
1. `data/ヨコレイ在庫_スプレッドシート投入用.xlsx` をGoogleドライブにアップロードし、Googleスプレッドシートとして保存
...
- `data/ヨコレイ在庫_スプレッドシート投入用.xlsx` の再生成・再変換はしない
